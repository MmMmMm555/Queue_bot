...
def command_start(update: Update, context: CallbackContext) -> None:
    ...
    update.message.reply_text(text="Assalomu alaykum botimizga hush kelibsiz !",)
    if created:
...
def name2(update: Update, context: CallbackContext) -> None:
    ...
    u.last_name = name2
    u.save()
    update.message.reply_text(text="Telefon raqamingizni kiriting", reply_markup=send_contact,)
    return states.PHONE
...
def phone(update: Update, context: CallbackContext) -> None:
    ...
    u.save()
    print(last_order)
    update.message.reply_text(text=f"Sizning tartib raqamingiz - {last_order}", reply_markup=ReplyKeyboardRemove())
    update.message.reply_text(text="kanalga qo'shiling https://t.me/Asadbek_Jumayev",)
